...
リモート管理（SSHなど）を行うために、SVI（VLANインターフェース）にIPアドレスを設定する。
...
VTY（リモートログイン用の仮想回線）でSSHのみを許可する。
...
## SSH
rsa鍵を生成する前に、ホスト名とドメイン名を設定しておく必要がある。
鍵のラベルにFQDN(ホスト名+ドメイン名)を使用するため。
...
## スタティックルート
※純粋なL2SWにこのコマンドはない。L3SWをL2SWとしてつかうときは、デフォルトゲートウェイの設定をこのコマンドでしてもいい。
